...
std::unordered_map<char, Piece> piece_to_board = {
    {'p', Piece(Pawn, black)},  {'b', Piece(Bishop, black)},
    {'r', Piece(Rook, black)},  {'n', Piece(Knight, black)},
    {'q', Piece(Queen, black)}, {'k', Piece(King, black)},

    {'_', Piece(none, white)},

    {'P', Piece(Pawn, white)},  {'B', Piece(Bishop, white)},
    {'R', Piece(Rook, white)},  {'N', Piece(Knight, white)},
    {'Q', Piece(Queen, white)}, {'K', Piece(King, white)}};
...
void board_from_fen(std::string fen) {
  ...
  std::string fenn = "4kb1r/p4ppp/4q3/8/8/1B6/PPP2PPP/2KR4";
  std::regex re("[\\w-]+");

  std::sregex_iterator begin =
      std::sregex_iterator(fenn.begin(), fenn.end(), re);
