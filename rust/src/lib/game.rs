...
impl Game {
    ...
    fn _is_legal_move(&self, move_: &Move) -> bool {
        ...
        if move_.is_castle {
            if board_copy.is_attacked(board_copy.find_king(self.turn), self.turn.opposite(), &self)
            {
                return false;
            }
            // Move king
    ...
    pub fn unmake_move(&mut self) {
        ...
        *self.board.get_mutable_en_passant() = last_move.old_en_passant;
        ...
        // Optional debug: verify board consistency
        #[cfg(debug_assertions)]
        {
            let king_pos = self.board.find_king(self.turn);
            assert!(king_pos.is_valid(), "King not found after unmake!");
        }
    }
    // pub fn unmake_move(&mut self) {
    //     let last_move = self.last_move().unwrap().clone();
    //     if last_move.get_piece().piece_type == PAWN || last_move.captured.is_some() {
    ...
    //             .to_pos
    //             .get_row_index()
    //             .abs_diff(last_move.from_pos.get_row_index())
    //             == 2)
    //     {
    //         let en_passant_square = Square::new(
    //             last_move.from_pos.get_col_index(),
    //             (last_move.from_pos.get_row_index() - last_move.to_pos.get_row_index()) / 2,
    //         );
    ...
    //         if last_move.piece.color == WHITE {
    //             self.castling.white_kingside = true;
    //             self.castling.white_queenside = true;
    //         } else {
    //             self.castling.black_kingside = true;
    //             self.castling.black_queenside = true;
    //         }
    //     } else {
    //         // Handle en passant capture
    //         if last_move.is_en_passant {
    ...
    //                 captured_pos,
    //                 Piecee::new(PAWN, last_move.piece.color.opposite()),
    //             );
    //         }
    //
    //         // Move piece
    ...
    //             self.board
    //                 .set_piece_at_square(last_move.from_pos, last_move.piece);
    //             self.board.remove_piece_at_square(
    //                 last_move.to_pos,
    //                 Piecee::new(last_move.promotion.expect("Promotion is some"), self.turn),
    //             );
    //             // self.board.set_piece(move.to_pos, Piecee(move.promotion, self.turn))
    //         }
    //
    //         // Update castling rights
    //         if last_move.piece.piece_type == KING {
    //             if self.turn == WHITE {
    //                 self.castling.white_kingside = true;
    //                 self.castling.white_queenside = true;
    //             } else {
    ...
    //                 self.castling.black_queenside = true;
    //             }
    //         } else if last_move.piece.piece_type == ROOK {
    //             // Queenside rook
    //             if last_move.from_pos.get_col_index() == 0 {
    //                 if self.turn == WHITE && last_move.from_pos.get_row_index() == 7 {
    //                     self.castling.white_queenside = true;
    //                 } else if self.turn == BLACK && last_move.from_pos.get_row_index() == 0 {
    //                     self.castling.black_queenside = true;
    //                 }
    //             } else if last_move.from_pos.get_col_index() == 7 {
    //                 // Kingside rook
    //                 if self.turn == WHITE && last_move.from_pos.get_row_index() == 7 {
    //                     self.castling.white_kingside = true;
    //                 } else if self.turn == BLACK && last_move.from_pos.get_row_index() == 0 {
    ...
    //     match from_piece {
    //         Some(p) => println!("Found {:?}", p),
    //         None => {
    //             println!("Didnt found piece at sqaure {}", move_.from_pos.to_uci());
    //             self.board.print();
    //         }
    //     }
    //
    ...
    //     self.hash = Some(h);
    //
    //     // --- Optional debug check (remove in release) ---
    //     #[cfg(debug_assertions)]
    //     {
    //         let check_hash = self.compute_hash(zobrist);
    //         assert_eq!(
    //             h,
    //             check_hash,
