...
impl Move {
    ...
    pub fn from_uci(uci: &str, game: &Game) -> Option<Move> {
        ...
        let old_en_passant = game.board.get_en_passant_bitboard(); // assume field exists
        let old_half_move = game.half_move_clock; // assume field exists

        // Return the matching move or None
        if possible_moves.len() == 1 {
            let mv = possible_moves[0];
            Some(Move::new(
                mv.from_pos,
                mv.to_pos,
                mv.piece,
